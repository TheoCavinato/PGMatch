...
#SBATCH --output /scratch/tcavinat/Phenotype_inference_attack/Effect_of_pheno_nbr_no_BMI/logs/%x_%A-%a.out
#SBATCH --error  /scratch/tcavinat/Phenotype_inference_attack/Effect_of_pheno_nbr_no_BMI/logs/%x_%A-%a.err
...
for ITR in {1..100}; do
	...
	#awk -F '\t' 'NR>1{ if($1 != 30180 && $1!=21001) {print}}' $VAR_EXPL_NO_CORR | head -n $NBR_PHENO > $USED_PHENO  # remove lymphocyte percentage (because we already have lymphocyte count)  and remove BMI to see its impact on our results
	awk -F '\t' 'NR>1{ if($1 != 30180 && $1!=21001 && $1!=30280) {print}}' $VAR_EXPL_NO_CORR | head -n $NBR_PHENO > $USED_PHENO  # remove lymphocyte percentage (because we already have lymphocyte count)  and remove BMI to see its impact on our results
