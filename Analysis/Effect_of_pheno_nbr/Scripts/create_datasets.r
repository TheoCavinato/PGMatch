...
#------------------------------------------------------------------------------#
for(itr in c(1:100)){
	out_pheno_train = gsub("itr_100", paste0("itr_",itr), args$out_pheno_train)
	out_pgs_train = gsub("itr_100", paste0("itr_",itr), args$out_pgs_train)
	out_pheno_test = gsub("itr_100", paste0("itr_",itr), args$out_pheno_test)
	out_pgs_test = gsub("itr_100", paste0("itr_",itr), args$out_pgs_test)
	out_pgs_train_h0 = gsub("itr_100", paste0("itr_",itr), args$out_pgs_train_h0)
	out_pgs_test_h0 = gsub("itr_100", paste0("itr_",itr), args$out_pgs_test_h0)
	...
	rows_train = c(1:args$n_train); rows_test = c((args$n_train+1):(args$n_train+args$n_test))
	...
	set_train_pgs_h0 = set_train_pgs[c(nrow(set_train_pgs), 1:(nrow(set_train_pgs)-1)), ]
	set_test_pgs_h0 = set_test_pgs[c(nrow(set_test_pgs), 1:(nrow(set_test_pgs)-1)), ]
	...
	write_df(set_train_pheno, out_pheno_train); write_df(set_train_pgs, out_pgs_train)
	write_df(set_test_pheno, out_pheno_test); write_df(set_test_pgs, out_pgs_test)
	write_df(set_train_pgs_h0, out_pgs_train_h0)
	write_df(set_test_pgs_h0, out_pgs_test_h0)
	cat("Iteration:", itr, "written.", fill=T)
}
