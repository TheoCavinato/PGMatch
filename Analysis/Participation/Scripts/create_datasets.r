...
parser$add_argument("--n_test", required=T, type="integer") # the number of people in the biobank
...
#args$info_p = "/scratch/tcavinat/Phenotype_inference_attack/Case_scenario_1_biobank/Analysis.NBR_PHENO_2.N_TEST_100000.N_TRAIN_1000/Sort_phenos_by_var/phenos.txt"
#args$n_traing = 1000
#args$n_test = 1000
#args$seed=1
...
# need to do the normalization now as we will not be able to do it later on
#(because the dataframe of phenotypes will only contain the same individual
#multiple times)
...
norm_set_test_pgs = apply(set_test_pgs[,-c('IID')], 2, quantile_norm)
norm_set_test_pheno = apply(set_test_pheno[,-c('IID')], 2, quantile_norm)
set_test_pgs = data.table(IID=set_test_pgs$IID, norm_set_test_pgs)
set_test_pheno = data.table(IID=set_test_pheno$IID, norm_set_test_pheno)
...
#write_df(new_set_test_pheno, args$out_pheno_test); write_df(new_set_test_pgs, args$out_pgs_test)
