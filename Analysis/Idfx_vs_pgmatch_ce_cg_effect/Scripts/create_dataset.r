...
parser$add_argument("--ori_ce", required = T)
parser$add_argument("--ori_cg", required = T)
...
ori_ce = as.matrix(read.table(args$ori_ce))
Ce <- as.matrix(nearPD(ori_ce, corr = TRUE)$mat)
ori_cg = as.matrix(read.table(args$ori_cg))
Cg <- as.matrix(nearPD(ori_cg, corr = TRUE)$mat)
